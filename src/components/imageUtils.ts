...
export const cropGridSquares = (imageTensor: tf.Tensor3D, boundingBox: FourNumbers): tf.Tensor4D =>
  tf.tidy(() => {
    // imageTensor is assumed to have HWC shape (height, width, channels)
    ...
    const cropSize: [number, number] = [20, 20]
    ...
    const imageIndices = Array(normalisedGridSquareBoundingBoxes.length).fill(0)
